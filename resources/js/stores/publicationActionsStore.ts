...
interface PublicationActionsStore {
  ...
  setItemLoading: (
    itemId: number,
    key: keyof LoadingStates[number],
    value: boolean,
  ) => void;
...
export const usePublicationActionsStore = create<PublicationActionsStore>(
  (set, get) => ({
    loadingStates: {},
    ...
    submitForApproval: async (itemId) => {
      ...
      try {
        ...
        const approvalRequest =
          response.data?.data?.request ?? response.data?.request;
        const publication =
          response.data?.data?.publication ?? response.data?.publication;

        // Actualizar el store de publicaciones con el nuevo estado
        if (publication) {
          const { usePublicationStore } =
