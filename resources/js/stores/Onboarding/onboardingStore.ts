...
import {
  ...
  isOnline,
  retryWithBackoff,
} from '@/Utils/common/networkErrorHandler';
import type { RetryOptions } from '@/Utils/common/networkErrorHandler';
  ...
  return {
    ...
    nextTourStep: () => {
      ...
      if (currentStep >= MAX_TOUR_STEPS) {
        console.warn(
          `Already past last tour step. Current step: ${currentStep}, Max steps: ${MAX_TOUR_STEPS}. Should call completeTour instead.`,
        );
