        ...
        <div className="bg-white rounded-lg border border-gray-200 p-6">
            ...
            {(addonTotal > 0 || isExceedingPlan) && (
                <div className="border-t pt-4">
                    ...
                    <div className="flex justify-between text-xs text-orange-600 mt-1">
                        <span>
                            {formatValue(addonUsed)} usado de addons
                        </span>
                        <span>
                            {formatValue(addonRemaining)} restante
                        </span>
                    </div>
                    ...
                    <div className="mt-3 p-3 bg-orange-50 rounded-md">
                        <p className="text-xs text-orange-800">
                            {isExceedingPlan ? (
                                <>
                                    <strong>Usando addons:</strong> Has excedido tu plan base por {formatValue(excessUsage)}. 
                                    Los addons se están consumiendo automáticamente.
                                </>
                            ) : (
                                <>
                                    <strong>Addons disponibles:</strong> Tus addons se activarán automáticamente 
                                    cuando excedas el límite de tu plan base ({isUnlimited ? 'ilimitado' : formatValue(planLimit)}).
                                </>
                            )}
                        </p>
                    </div>
            ...
            <div className="mt-4 pt-4 border-t bg-gray-50 -mx-6 -mb-6 px-6 pb-6 rounded-b-lg">
                ...
                <div className="text-xs text-gray-500 mt-1">
                    Plan: {isUnlimited ? 'Ilimitado' : formatValue(planLimit)} + 
                    Addons: {formatValue(addonTotal)}
                </div>
            </div>
        </div>
    );
