            ...
            <div className="mb-4 p-3 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                <div className="flex items-center justify-between mb-2">
                    ...
                    <span className={`text-xs font-bold px-2 py-1 rounded-full ${
                        addon.percentage > 90 ? 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-200' :
                        addon.percentage > 75 ? 'bg-orange-100 text-orange-800 dark:bg-orange-900/30 dark:text-orange-200' :
            ...
            <div className="pt-3 border-t border-gray-200 dark:border-gray-700">
                <div className="text-xs text-gray-500 dark:text-gray-400">
                    {addon.purchase_count && addon.purchase_count > 1 ? (
                        ...
                        <>
                            {t('subscription.addons.purchasedOn', 'Comprado el')}: {addon.first_purchased_at ? new Date(addon.first_purchased_at).toLocaleDateString() : 'N/A'}
                            {' • '}
                            <span className="font-semibold">${addon.price?.toFixed(2) || addon.total_price?.toFixed(2)}</span>
                        </>
                    )}
                </div>
                {addon.expires_at && (
