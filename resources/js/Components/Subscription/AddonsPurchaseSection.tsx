...
export function AddonsPurchaseSection({ addons }: AddonsPurchaseSectionProps) {
    ...
    const firstEnabledTab = 
        (addons.ai_credits?.enabled ? 'ai_credits' : 
        addons.storage?.enabled ? 'storage' : 
        addons.publications?.enabled ? 'publications' : 
        addons.team_members?.enabled ? 'team_members' : 
        'ai_credits') as 'ai_credits' | 'storage' | 'publications' | 'team_members';

    const [activeTab, setActiveTab] = useState<'ai_credits' | 'storage' | 'publications' | 'team_members'>(firstEnabledTab);
    ...
    const currentPackages = addons[activeTab]?.packages ? Object.values(addons[activeTab].packages).filter(pkg => pkg.enabled) : [];
        ...
        return (
            ...
            >
                ...
                <div className="flex justify-center mb-4">
                    {activeTab === 'ai_credits' && <Sparkles className="w-12 h-12 text-primary-500" />}
                    {activeTab === 'storage' && <HardDrive className="w-12 h-12 text-primary-500" />}
                    {activeTab === 'publications' && <FileText className="w-12 h-12 text-primary-500" />}
                    {activeTab === 'team_members' && <Users className="w-12 h-12 text-primary-500" />}
                </div>
                ...
                <div className="mb-4">
                    <div className="flex items-center justify-center gap-3">
                        ...
                        <Input
                            ...
                            type="number"
                            value={quantity}
                            onChange={(e) => setQuantities({ ...quantities, [pkg.sku]: Math.max(1, parseInt(e.target.value) || 1) })}
    ...
    const hasCurrencyConversion = firstPackage && firstPackage.currency && firstPackage.currency !== 'USD';
                        ...
                        <div className="flex items-start gap-3">
                            ...
                            <div className="flex-1">
                                <p className="text-sm text-blue-800 dark:text-blue-200">
                                    {t('subscription.addons.currencyInfo', 
                        ...
                        {currentPackages.length > 0 ? (
                            ...
                            <div className="col-span-full text-center py-12">
                                <p className="text-gray-500 dark:text-gray-400">
                                    {t('subscription.addons.noPackages', 'No hay paquetes disponibles en esta categoría')}
                                </p>
                            </div>
                        )}
                    </div>
