...
import { ChevronLeft, ChevronRight, Loader2, Filter, Calendar as CalendarIcon } from 'lucide-react';
import { FaInstagram, FaFacebook, FaTwitter, FaLinkedin, FaYoutube, FaTiktok } from 'react-icons/fa';
import axios from 'axios';
import { getPlatformConfig, SOCIAL_PLATFORMS } from '@/Constants/socialPlatforms';
...
export default function ModernCalendar({ onEventClick }: { onEventClick?: (id: number, type: 'publication' | 'post') => void }) {
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm overflow-hidden">
            <div className="p-6">
                ...
                <div className="flex flex-col sm:flex-row justify-between items-center mb-6 sm:mb-8 gap-4 sm:gap-6">
                    <div className="flex items-center gap-4">
                        <h3 className="text-lg sm:text-2xl font-bold text-gray-900 dark:text-white capitalize flex items-center gap-3">
                            ...
                            {loading && <Loader2 className="w-4 h-4 sm:w-5 sm:h-5 animate-spin text-primary-500" />}
                        </h3>
                    </div>
                    ...
                    <div className="flex flex-wrap items-center gap-2 sm:gap-3 w-full sm:w-auto justify-center sm:justify-end">
                        ...
                        <div className="flex items-center bg-gray-100 dark:bg-gray-800 rounded-lg p-1 overflow-x-auto scrollbar-subtle max-w-full sm:max-w-none">
                            {platforms.map(p => (
                                <button
                                    ...
                                    title={p}
                                >
                                    {p === 'all' ? <Filter className="w-3.5 h-3.5 sm:w-4 sm:h-4" /> : <PlatformIcon platform={p} className="w-3.5 h-3.5 sm:w-4 sm:h-4" />}
                                </button>
                            ))}
                        </div>
                        ...
                        <div className="flex items-center gap-1 bg-gray-100 dark:bg-gray-800 p-1 rounded-xl border border-gray-200 dark:border-gray-700">
                            <button onClick={prevMonth} className="p-1.5 sm:p-2 hover:bg-white dark:hover:bg-gray-700 rounded-lg transition-all text-gray-600 dark:text-gray-300">
                                <ChevronLeft className="w-4 h-4 sm:w-5 sm:h-5" />
                            </button>
                            <button onClick={goToToday} className="px-3 sm:px-4 py-1.5 sm:py-2 text-xs sm:text-sm font-semibold hover:bg-white dark:hover:bg-gray-700 rounded-lg transition-all text-gray-700 dark:text-gray-200">
                                Hoy
                            </button>
                            <button onClick={nextMonth} className="p-1.5 sm:p-2 hover:bg-white dark:hover:bg-gray-700 rounded-lg transition-all text-gray-600 dark:text-gray-300">
                ...
                <div className="w-full">
                    <div className="w-full border border-gray-200 dark:border-gray-800 rounded-xl overflow-hidden shadow-sm bg-gray-50 dark:bg-gray-900/50">
                        {/* Weekday Headers */}
                        <div className="grid grid-cols-7 bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800">
                            {['Dom', 'Lun', 'Mar', 'Mié', 'Jue', 'Vie', 'Sáb'].map(day => (
                                <div key={day} className="py-3 text-center text-[10px] sm:text-xs font-bold uppercase tracking-wider text-gray-400 dark:text-gray-500">
                        ...
                        <div className="grid grid-cols-7 auto-rows-fr min-h-[300px] sm:min-h-[700px] bg-gray-200 dark:bg-gray-800 gap-px">
                            ...
                            {startingEmptySlots.map((_, i) => (
                                <div key={`empty-${i}`} className="bg-gray-50/50 dark:bg-gray-900/50 p-1 sm:p-2"></div>
                            ))}
                                        ...
                                        <div className="flex justify-between items-start mb-1 sm:mb-2">
                                            <span className={`
                                                ...
                                                ${isTodayDay
                                                    ...
                                                    : isSelected ? 'bg-primary-100 dark:bg-primary-900/40 text-primary-600 dark:text-primary-400' : 'text-gray-500 dark:text-gray-400 group-hover:bg-gray-100 dark:group-hover:bg-gray-800'}
                                            `}>
                                                {format(day, 'd')}
                                            </span>
                                            {isTodayDay && <span className="hidden sm:inline text-[10px] font-bold text-primary-600 dark:text-primary-400 uppercase">Hoy</span>}
                                        </div>
                                                    ...
                                                    >
                                                        ...
                                                        <div className="p-2 pl-3 flex items-start gap-2">
                                                            ...
                                                            <div className="min-w-0 flex-1">
                                                                <div className="text-xs font-bold text-gray-700 dark:text-gray-200 truncate leading-tight">{event.title}</div>
                    ...
                    <div className="lg:hidden mt-6 space-y-4">
                        <div className="flex items-center justify-between">
                            ...
                            <span className="text-[10px] font-bold uppercase tracking-widest text-gray-400">
                                {filteredEvents.filter(e => isSameDay(parseISO(e.start), selectedDate)).length} Eventos
                            </span>
                        </div>

                        <div className="space-y-3">
                            {filteredEvents.filter(e => isSameDay(parseISO(e.start), selectedDate)).length > 0 ? (
                                filteredEvents.filter(e => isSameDay(parseISO(e.start), selectedDate)).map(event => (
                                    <div
                                        key={event.id}
                                        onClick={() => {
                                            const pubId = event.extendedProps.publication_id || event.resourceId;
                                    ...
                                    >
                                        <div className="w-12 h-12 rounded-xl flex-shrink-0 flex items-center justify-center bg-white dark:bg-neutral-800 shadow-sm border border-gray-100 dark:border-neutral-700">
                                            <PlatformIcon platform={event.extendedProps.platform} className="w-6 h-6" />
                                        </div>
                                        <div className="flex-1 min-w-0">
                                            <h5 className="font-bold text-gray-900 dark:text-white truncate">{event.title}</h5>
                                            <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                                                {format(parseISO(event.start), 'HH:mm')} • <span className="capitalize">{event.status}</span>
                                            </p>
                                        </div>
                                        <div className="w-2 h-10 rounded-full" style={{ backgroundColor: event.color }} />
                                    </div>
                                ))
                            ) : (
