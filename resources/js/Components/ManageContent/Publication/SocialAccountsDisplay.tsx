...
const SocialAccountsDisplay = memo(({
  ...
  const displayItems = useMemo(() => {
    ...
    const scheduledPosts = Array.isArray(publication.scheduled_posts) ? publication.scheduled_posts : [];
    const postLogs = Array.isArray(publication.social_post_logs) ? publication.social_post_logs : [];
  ...
  const connectedIds = useMemo(() => new Set(connectedAccounts.map(a => a.id)), [connectedAccounts]);
  ...
  return (
    <div className="flex flex-wrap gap-1.5 items-center">
      {displayItems.slice(0, 4).map((item) => {
        ...
        const isConnected = item.social_account_id ? connectedIds.has(item.social_account_id) : false;
