...
import { Edit, Folder, Image, Rocket, Trash2, Video } from "lucide-react";
              ...
              <div className="flex items-start gap-4 mb-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between gap-2 mb-1">
                    <span className={`inline-flex items-center px-2 py-0.5 rounded-lg text-[10px] font-bold uppercase tracking-tight ${getStatusColor(item.status)}`}>
                      {item.status || "Draft"}
                    </span>
                    {mediaCount.total > 0 && (
                      <div className="flex items-center gap-1.5 opacity-60">
                        {mediaCount.images > 0 && <Image className="w-3 h-3" />}
                        {mediaCount.videos > 0 && <Video className="w-3 h-3" />}
                      </div>
                    )}
                  </div>
                  <h3 className="font-bold text-gray-900 dark:text-white text-lg truncate leading-snug" title={item.title || t("publications.table.untitled")}>
              ...
              <div className="flex flex-wrap gap-2 py-3 border-y border-gray-50 dark:border-neutral-800/50 mb-4">
                ...
                {item.campaigns && item.campaigns.length > 0 && (
                  <div className="flex items-center gap-1.5 px-2 py-1 rounded-lg bg-indigo-50/50 dark:bg-indigo-900/10 border border-indigo-100/50 dark:border-indigo-900/30">
                    ...
                    <span className="text-[10px] font-bold text-indigo-600 dark:text-indigo-400">{item.campaigns.length}</span>
                  </div>
                )}
              </div>
            </div>
