...
export default function SystemStatusCard({ status }: Props) {
    ...
    return (
        <Card className="bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700">
            <CardHeader>
                <CardTitle className="text-gray-900 dark:text-gray-100">{t('admin.system_status.title')}</CardTitle>
            ...
            <CardContent className="space-y-4">
                {status.general.maintenance_mode && (
                    <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-3 flex items-center gap-2">
                        ...
                        <div>
                            <p className="text-sm font-semibold text-red-900 dark:text-red-100">{t('admin.system_status.maintenance_mode.title')}</p>
                            <p className="text-xs text-red-700 dark:text-red-300">{t('admin.system_status.maintenance_mode.description')}</p>
                        </div>
                    </div>
                )}
                ...
                {!status.general.new_registrations && (
                    <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg p-3 flex items-center gap-2">
                        ...
                        <div>
                            <p className="text-sm font-semibold text-yellow-900 dark:text-yellow-100">{t('admin.system_status.registrations_disabled.title')}</p>
                            <p className="text-xs text-yellow-700 dark:text-yellow-300">{t('admin.system_status.registrations_disabled.description')}</p>
                        </div>
                    </div>
                )}
                ...
                <div className="pt-4 border-t border-gray-200 dark:border-gray-700">
                    <div className="grid grid-cols-2 gap-4 text-center">
                        <div>
                            ...
                            <p className="text-xs text-gray-600 dark:text-gray-400">{t('admin.system_status.stats.active_features')}</p>
                        </div>
                        <div>
                            <p className="text-2xl font-bold text-blue-600 dark:text-blue-400">
                                {countEnabled(status.plans)}
                            </p>
                            <p className="text-xs text-gray-600 dark:text-gray-400">{t('admin.system_status.stats.available_plans')}</p>
                        </div>
                    </div>
                </div>
            </CardContent>
