...
const CustomTimeSelector = ({
  ...
  const incH = () => {
    const h = (parseInt(hours) + 1) % 24;
    setHours(h.toString().padStart(2, '0'));
    updateTime(h, parseInt(minutes) || 0);
  };
  const decH = () => {
    const h = parseInt(hours) === 0 ? 23 : parseInt(hours) - 1;
    setHours(h.toString().padStart(2, '0'));
    updateTime(h, parseInt(minutes) || 0);
  };
  const incM = () => {
    const m = (parseInt(minutes) + 1) % 60;
    setMinutes(m.toString().padStart(2, '0'));
    updateTime(parseInt(hours) || 0, m);
  };
  const decM = () => {
    const m = parseInt(minutes) === 0 ? 59 : parseInt(minutes) - 1;
    setMinutes(m.toString().padStart(2, '0'));
    updateTime(parseInt(hours) || 0, m);
  };
...
const DatePickerModern = <T extends FieldValues>({
  ...
  const handleCalendarChange = (val: CalendarDate | null) => {
    ...
    if (showTimeSelect && displayDate) {
      ...
      const n = new Date();
      newDate.setHours(n.getHours());
      newDate.setMinutes(n.getMinutes());
    }
  ...
  const minCalendarDate = allowPastDates
    ? undefined
    : minDate
