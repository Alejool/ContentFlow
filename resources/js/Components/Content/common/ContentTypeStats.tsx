  ...
  const stats = useMemo(() => {
    ...
    return Object.entries(typeCount)
      .map(([type, count]) => ({
        ...
        display:
          CONTENT_TYPE_DISPLAY[type as keyof typeof CONTENT_TYPE_DISPLAY] ||
          ...
          return (
            <div key={type} className="flex items-center justify-between">
              <div className="flex flex-1 items-center gap-3">
                <div
                  className={`inline-flex items-center gap-1.5 rounded-full px-2 py-1 text-xs font-bold ${display.color} ${display.borderColor} border`}
                >
                  {IconComponent && (
                    <IconComponent className="h-3 w-3" aria-label={display.label} />
                  )}
