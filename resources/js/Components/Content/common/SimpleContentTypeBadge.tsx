...
const SimpleContentTypeBadge = memo(({
  ...
  return (
    <div className={`
      ...
      border ${borderColor} ${color} ${sizeClasses[size]}
      ${className}
    `}>
      {IconComponent && (
        <IconComponent className={iconSizes[size]} aria-label={labels[type as keyof typeof labels]} />
      )}
