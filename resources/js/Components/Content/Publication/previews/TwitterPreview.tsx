      ...
      <div className="flex gap-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-1.5 text-[15px] leading-5">
            ...
            <span className="truncate text-gray-500 dark:text-neutral-500">
              @{user?.username || 'username'}
            </span>
            <span className="text-gray-500 dark:text-neutral-500">·</span>
            <span className="text-gray-500 hover:underline dark:text-neutral-500">
              {date ? format(date, 'MMM d') : 'Now'}
            </span>
          </div>

          <div className="mt-1 text-[15px] leading-normal break-words whitespace-pre-wrap text-gray-900 dark:text-neutral-100">
            {content || <span className="text-gray-400 italic">Start typing to preview...</span>}
          </div>

          {mediaUrls.length > 0 && contentType !== 'poll' && (
            <div
              className={`mt-3 grid gap-0.5 overflow-hidden rounded-lg border border-gray-200 dark:border-neutral-800 ${
                mediaUrls.length === 1
                  ? 'grid-cols-1'
                  : mediaUrls.length === 2
                    ? 'grid-cols-2'
            ...
            >
              {mediaUrls.slice(0, 4).map((url, index) => (
                <div key={index} className="relative aspect-[16/9] bg-gray-100 dark:bg-neutral-900">
          ...
          {contentType === 'poll' && pollOptions.length >= 2 && (
            <div className="mt-3 overflow-hidden rounded-lg border border-gray-200 dark:border-neutral-800">
              <div className="border-b border-gray-200 bg-gray-50 px-3 py-2 dark:border-neutral-800 dark:bg-neutral-900/30">
              ...
              {pollOptions
                ...
                .map((option, index) => (
                  <div
                    key={index}
                    className="flex cursor-pointer items-center justify-between border-b border-gray-200 p-3 transition-colors last:border-b-0 hover:bg-gray-50 dark:border-neutral-800 dark:hover:bg-gray-900/50"
                  >
                    <div className="flex flex-1 items-center gap-3">
                      ...
                      <span className="text-[15px] text-gray-900 dark:text-neutral-100">{option}</span>
                    </div>
                    <span className="text-sm text-gray-500 dark:text-neutral-400">0%</span>
                  </div>
                ))}
            </div>
          )}
