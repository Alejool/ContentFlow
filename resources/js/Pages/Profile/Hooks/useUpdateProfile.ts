import { useForm as useHookForm, UseFormRegister, UseFormHandleSubmit, FieldErrors, UseFormSetError } from 'react-hook-form';
...
export const useUpdateProfile = (): UseUpdateProfileReturn => {
    ...
    const updateProfile = async (data: ProfileFormData): Promise<UpdateProfileResult> => {
        try {
            const response = await axios.patch<ApiResponse>(route('profile.update'), data);
            ...
            if (axiosError.response?.data?.errors) {
                ...
                toast.error(axiosError.response?.data?.message || 'An error occurred while updating profile');
            }
            return { error: true };
        }
    };
