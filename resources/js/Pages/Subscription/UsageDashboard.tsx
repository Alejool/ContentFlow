...
export default function UsageDashboard({ subscription, usage }: Props) {
  ...
  const getMetricName = (type: string) => {
    switch (type) {
      case 'publications': return t('subscription.usage.publications');
      case 'storage': return t('subscription.usage.storage');
      case 'ai_requests': return t('subscription.usage.aiRequests');
      default: return type;
    }
  };
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <Badge variant={getPlanBadgeColor(subscription.plan)} className="text-lg px-4 py-2">
          {t('subscription.billing.plan')} {subscription.plan.charAt(0).toUpperCase() + subscription.plan.slice(1)}
        </Badge>
      </div>
        ...
        <Card className="bg-gradient-to-r from-blue-50 to-indigo-50 border-blue-200">
          ...
          <CardContent className="flex gap-4">
            <Button 
              ...
              disabled={isLoading}
            >
              {t('pricing.plans.starter.name')} - $19/{t('pricing.billing.month')}
            </Button>
            <Button 
              onClick={() => handleUpgrade('professional')}
              variant="default"
              disabled={isLoading}
            >
              {t('pricing.plans.professional.name')} - $49/{t('pricing.billing.month')}
            </Button>
          </CardContent>
        </Card>
      )}
