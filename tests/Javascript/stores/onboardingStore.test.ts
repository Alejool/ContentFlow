...
describe("Onboarding Store Property Tests", () => {
  ...
  describe("Property 1: Tour Step Progression", () => {
    it("advancing tour step increments current step by one", () => {
      ...
      fc.assert(
        fc.property(
          fc.integer({ min: 0, max: 5 }), // current step below MAX_TOUR_STEPS (6) — store refuses to advance past it
          (currentStep) => {
            const store = useOnboardingStore.getState();
      ...
      fc.assert(
        fc.property(
          ...
          (currentStep) => {
            ...
            // Verify property: step never decreases (store clamps at MAX_TOUR_STEPS,
            // so steps >= 6 stay unchanged rather than incrementing)
