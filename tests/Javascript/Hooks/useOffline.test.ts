...
import { useOffline } from '../../../resources/js/Hooks/Offline/useOffline';
import type { QueuedOperation } from '../../../resources/js/types/common/optimistic';

// Mock the indexedDBQueue module
vi.mock('../../../resources/js/Utils/Offline/indexedDBQueue', () => ({
  indexedDBQueue: {
...
describe('useOffline Hook', () => {
  ...
  describe('Queue Operations (Subtask 8.2)', () => {
    it('should queue an operation', async () => {
      const { indexedDBQueue } = await import('../../../resources/js/Utils/Offline/indexedDBQueue');
      const { result } = renderHook(() => useOffline());
