...
use App\Notifications\Media\MediaUploadProcessed;
...
class ProcessBackgroundUpload implements ShouldQueue
{
    ...
    try {
      try {
        ...
        if ($this->mediaFile->file_type === 'video' 
          ...
          \App\Jobs\Media\GenerateReelsFromVideo::dispatch(
            $this->mediaFile->id,
            $this->publication->id,
            [
  ...
  private function broadcastProgress(int $percentage, string $message = ''): void
  {
    try {
      ...
      broadcast(new \App\Events\Publication\ProcessingProgressUpdated(
        $userId,
        $jobId,
        $this->publication->id,
        [
  ...
  private function broadcastCompletion(string $status, string $errorMessage = ''): void
  {
    try {
      ...
      broadcast(new \App\Events\Media\VideoProcessingCompleted(
        $userId,
        $this->publication->id,
        $status,
        $errorMessage
