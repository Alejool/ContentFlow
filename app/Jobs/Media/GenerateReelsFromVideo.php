...
use App\Notifications\Media\ReelsGenerationCompleted;
...
class GenerateReelsFromVideo implements ShouldQueue
{
  ...
  public function handle(
    ...
    try {
      ...
      if ($this->publicationId) {
        broadcast(new \App\Events\Media\VideoProcessingCompleted($userId, $this->publicationId, 'completed'));
      }
      ...
      // Broadcast failure
      if ($this->publicationId) {
        broadcast(new \App\Events\Media\VideoProcessingCompleted($userId, $this->publicationId, 'failed', $e->getMessage()));
      }
  ...
  public function failed(\Throwable $exception): void
  {
    ...
    if ($mediaFile) {
      ...
      // Broadcast failure
      if ($this->publicationId) {
        broadcast(new \App\Events\Media\VideoProcessingCompleted($userId, $this->publicationId, 'failed', $exception->getMessage()));
      }
    }
  }
  ...
  private function updateProgress(
    ...
    if ($this->publicationId) {
      broadcast(new \App\Events\Publication\ProcessingProgressUpdated($userId, $jobId, $this->publicationId, $data));
    }
  }
