...
class BulkPublishPublications implements ShouldQueue
{
    ...
    private function notifyBatchStarted(string $batchId, int $jobCount, string $planSlug): void
    {
        try {
            ...
            if ($user) {
                $user->notify(new \App\Notifications\Publication\BulkPublishStartedNotification(
                    $batchId,
                    $jobCount,
                    $planSlug,
                    $this->workspaceId
    ...
    private function notifyCompletion(bool $success, ?string $error = null): void
    {
        try {
            ...
            if ($user) {
                $user->notify(new \App\Notifications\Publication\BulkPublishCompletedNotification(
